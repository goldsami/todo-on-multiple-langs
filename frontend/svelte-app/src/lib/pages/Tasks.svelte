<script lang="ts">
  ...
  const updateTaskMutation = useMutation(
    ({ id, user, ...task }: Partial<Task>) => {
      return axios.put(`http://localhost:4000/api/tasks/${id}`, {
        ...task,
        time: new Date(task.time).toISOString(),
      });
    },
  ...
  $: filteredTasks = sortTasks(
    filterTasks($queryResult?.data?.data, currentTab)
  );
</script>
...
{:else}
  ...
  {/each}
{/if}
<Button class="m-4" on:click={() => (modalState.show = true)}
  >Create task</Button
>
<MutateTaskModal
  on:close={closeModal}
